...
class MuscleBuilding extends StatefulWidget {
  const MuscleBuilding({Key? key}) : super(key: key);

  @override
  _MuscleBuildingState createState() => _MuscleBuildingState();
}

class _MuscleBuildingState extends State<MuscleBuilding> {
  List<Map<String, dynamic>> muscleBuildingExercises = [];
  late mongo.Db db;
  ...
  Future<void> fetchDataFromDatabase() async {
    ...
    const collectionName = 'muscle_building';
    ...
    try {
      ...
      setState(() {
        muscleBuildingExercises = fetchedData.map((doc) => {
          'day': doc['day'] ?? 'Unknown',
          'exercise': doc['exercise'] ?? 'Unknown',
          'sets_reps': doc['sets_reps'] ?? 'Unknown',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: muscleBuildingExercises.isEmpty
        ...
        child: Padding(
          ...
          child: Table(
            ...
            children: [
              ...
              for (var exercise in muscleBuildingExercises)
                TableRow(
                  decoration: BoxDecoration(
                    color: muscleBuildingExercises.indexOf(exercise) % 2 == 0
                        ? Colors.white
                        : Colors.blue[50],
                  ),
                  children: [
